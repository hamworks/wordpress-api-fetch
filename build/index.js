...
function createNonceMiddleware(nonce) {
  function middleware(options, next) {
    ...
    return next(_objectSpread$1({}, options, {
      headers: _objectSpread$1({}, headers, {
...
var namespaceAndEndpointMiddleware = function namespaceAndEndpointMiddleware(options, next) {
  ...
  return next(_objectSpread$2({}, options, {
    path: path
  }));
};
...
var createRootURLMiddleware = function createRootURLMiddleware(rootURL) {
  return function (options, next) {
    return namespaceAndEndpointMiddleware(options, function (optionsWithPath) {
      ...
      return next(_objectSpread$3({}, optionsWithPath, {
        url: url
      }));
    });
  };
...
function addQueryArgs() {
  ...
  if (queryStringIndex !== -1) {
    ...
    args = Object.assign(lib_2(url.substr(queryStringIndex + 1)), args); // Change working base URL to omit previous query arguments.

    baseUrl = baseUrl.substr(0, queryStringIndex);
  }

  return baseUrl + '?' + lib_3(args);
}
...
function getQueryArg(url, arg) {
  var queryStringIndex = url.indexOf('?');
  var query = queryStringIndex !== -1 ? lib_2(url.substr(queryStringIndex + 1)) : {};
  return query[arg];
}
...
var modifyQuery = function modifyQuery(_ref, queryArgs) {
  var path = _ref.path,
      url = _ref.url,
      options = _objectWithoutProperties$1(_ref, ["path", "url"]);

  return _objectSpread$4({}, options, {
...
var fetchAllMiddleware = /*#__PURE__*/function () {
  var _ref2 = _asyncToGenerator( /*#__PURE__*/regenerator.mark(function _callee(options, next) {
    ...
    return regenerator.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          ...
          case 4:
            _context.next = 6;
            return next(_objectSpread$4({}, modifyQuery(options, {
              per_page: 100
            }), {
          ...
          case 16:
            ...
            _context.next = 19;
            return next(_objectSpread$4({}, options, {
...
function httpV1Middleware(options, next) {
  ...
  if (OVERRIDE_METHODS.has(method.toUpperCase())) {
    options = _objectSpread$5({}, options, {
      headers: _objectSpread$5({}, options.headers, {
...
var parseResponse$1 = function parseResponse(response) {
...
var parseResponseAndNormalizeError = function parseResponseAndNormalizeError(response) {
  var shouldParseResponse = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : true;
  return Promise.resolve(parseResponse$1(response, shouldParseResponse)).catch(function (res) {
...
function mediaUploadMiddleware(options, next) {
  ...
  return next(_objectSpread$6({}, options, {
    parse: false
...
var middlewares = [userLocaleMiddleware, namespaceAndEndpointMiddleware, httpV1Middleware, fetchAllMiddleware];
...
var defaultFetchHandler = function defaultFetchHandler(nextOptions) {
  var url = nextOptions.url,
      ...
      remainingOptions = _objectWithoutProperties$1(nextOptions, ["url", "path", "data", "parse"]);

  var body = nextOptions.body,
      headers = nextOptions.headers; // Merge explicitly-provided headers with default values.

  headers = _objectSpread$7({}, DEFAULT_HEADERS, {}, headers); // The `data` property is a shorthand for sending a JSON body.
  ...
  var responsePromise = window.fetch(url || path, _objectSpread$7({}, DEFAULT_OPTIONS$1, {}, remainingOptions, {
    body: body,
    headers: headers
  }));
...
function useApiFetch(path) {
  ...
  react.useEffect(function () {
    ...
    apiFetch({
      ...
      setData(fetchedData); // We've stopped fetching

      setIsLoading(false);
    }).catch(function (err) {
      setError(err); // We've stopped fetching
 ...
 * @return {Promise} Parsed response
 */
const parseResponse$2 = ( response, shouldParseResponse = true ) => {
	if ( shouldParseResponse ) {
		if ( response.status === 204 ) {
			return null;
		}
...
const parseJsonAndNormalizeError$1 = ( response ) => {
 ...
 * @return {Promise} Parsed response.
 */
const parseResponseAndNormalizeError$1 = (
	response,
	shouldParseResponse = true
) => {
	return Promise.resolve(
		parseResponse$2( response, shouldParseResponse )
	).catch( ( res ) => parseAndThrowError$1( res, shouldParseResponse ) );
};

function parseAndThrowError$1( response, shouldParseResponse = true ) {
	if ( ! shouldParseResponse ) {
		throw response;
	}

	return parseJsonAndNormalizeError$1( response ).then( ( error ) => {
		const unknownError = {
			code: 'unknown_error',
			message: __( 'An unknown error occurred.' ),
		};
...
var checkStatus$1 = function checkStatus(response) {
  if (response.status >= 200 && response.status < 300) {
    return response;
  }
...
var fetchHandler$1 = function fetchHandler(nextOptions) {
  var url = nextOptions.url,
      path = nextOptions.path,
      data = nextOptions.data,
      _nextOptions$parse = nextOptions.parse,
      parse = _nextOptions$parse === void 0 ? true : _nextOptions$parse,
      remainingOptions = objectWithoutProperties(nextOptions, ["url", "path", "data", "parse"]);

  var body = nextOptions.body,
      headers = nextOptions.headers; // Merge explicitly-provided headers with default values.

  headers = _objectSpread$8(_objectSpread$8({}, DEFAULT_HEADERS$1), headers); // The `data` property is a shorthand for sending a JSON body.
  ...
  var responsePromise = globalThis.fetch(url || path, _objectSpread$8(_objectSpread$8(_objectSpread$8({}, DEFAULT_OPTIONS$2), remainingOptions), {}, {
    body: body,
    headers: headers
  }));
  return responsePromise // Return early if fetch errors. If fetch error, there is most likely no
  ...
  // the message might depend on the browser.
  .then(function (value) {
    return Promise.resolve(value).then(checkStatus$1)["catch"](function (response) {
      return parseAndThrowError$1(response, parse);
    }).then(function (response) {
      return parseResponseAndNormalizeError$1(response, parse);
    });
  }, function () {
    throw {
      code: 'fetch_error',
